...
void getFloatWithLayoutStr(NSString *str,float *value){
    if (str&&value) {
...
//hasAsso 是否存在关联
id getSingleInstanceWithLayoutStr(NSString *str,Class *mainClass)
{
    if (str) {
...
void getInstancesWithLayoutStr(NSString *str,NSDictionary **dict){
    ...
    for (__strong NSString *subStr in arr) {
...
#endif
            Class c=nil;
            id <QMLLayoutDelegate> ins=getSingleInstanceWithLayoutStr(subStr,&c);
            if (ins&&ins.flag) {
    ...
    id getInstanceWithLayoutStr(NSString *str,BOOL hasAsso)
    {
        if (!str) {
            return nil;
        }
        NSArray *arr=[str componentsSeparatedByString:LAYOUT_KEY_CLASS];
        NSMutableDictionary *dict=[NSMutableDictionary dictionary];
        Class c=nil;
#if __has_feature(objc_arc)
        for (__strong NSString *subStr in arr) {
            ...
            for (NSString *subStr in arr) {
                ...
                subStr = [LAYOUT_KEY_CLASS stringByAppendingString:subStr];
                id <QMLLayoutDelegate> ins=getSingleInstanceWithLayoutStr(subStr,&c);
        ...
        id getInstanceWithLayoutFile(NSString *name,BOOL hasAsso)
        {
        ...
        void getLayoutStrWithLayoutFile(NSString *name,NSString **str)
        {
            if(str)
            {
                ...
                if ([manager fileExistsAtPath:path]) {
                    *str=[NSString stringWithContentsOfFile:path encoding:4 error:nil];
                }
                if (!*str) {
